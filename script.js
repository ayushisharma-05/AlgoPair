...
function initializeSocket() {
    ...
    socket.on('waiting-for-stranger', ({ roomId }) => {
        currentRoomId = roomId;
        showWaitingMessage();
    });
...
// Friend connection functions
async function handleConnectWithFriend() {
    ...
    try {
        const response = await fetch('http://localhost:5000/api/rooms/generate-code', {
            method: 'POST'
        });
        const data = await response.json();
        
        if (response.ok) {
            socket.emit('create-friend-room', user.id);
...
// Stranger connection function
function handleConnectWithStranger() {
    const user = JSON.parse(localStorage.getItem('user'));
    if (!user) {
        alert('Please sign in first');
        window.location.href = 'signin.html';
    ...
    socket.emit('find-stranger', user.id);
}

// UI Helper functions
function showRoomCode(code) {
    // Create and show room code modal
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <h3>${code}</h3>
            <button onclick="copyRoomCode('${code}')">Copy Code</button>
...
function closeModal() {
    const modal = document.querySelector('.room-code-modal, .waiting-modal');
    if (modal) {
        modal.remove();
    }
}
...
function startCodingSession(users) {
    // Redirect to coding session page with room ID
    window.location.href = `coding-session.html?room=${currentRoomId}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const connectStrangerBtn = document.getElementById('connect-stranger');
    
    if (connectFriendBtn) {
        connectFriendBtn.addEventListener('click', handleConnectWithFriend);
    }
    
    if (connectStrangerBtn) {
        connectStrangerBtn.addEventListener('click', handleConnectWithStranger);
  }
});
